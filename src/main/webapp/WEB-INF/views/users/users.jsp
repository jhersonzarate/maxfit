<%-- ============================================================
     users.jsp  —  MaxFit Sistema de Gestión
     Módulo de gestión de usuarios del sistema (RF-14).

     Servlet:  UsersController.java  → GET/POST /users
     Acceso:   Solo ROL-ADMIN (RoleFilter)

     ── Parámetros de URL que activan vistas ──────────────────
       /users               → lista de usuarios (default)
       /users?form=true     → formulario nuevo / edición
...
<head>
    ...
    <c:choose>
        <c:when test="${param.form eq 'true'}">
            <c:set var="pageTitle" value="${modoEdicion ? 'Editar Usuario' : 'Nuevo Usuario'}" scope="request"/>
        </c:when>
        <c:otherwise>
    ...
    <style>
        ...
        .estado-cuenta.activo   .estado-cuenta__dot { background: var(--clr-success); }
        .estado-cuenta.activo   { color: var(--clr-success); }
        .estado-cuenta.inactivo .estado-cuenta__dot { background: var(--clr-text-dim); }
        .estado-cuenta.inactivo { color: var(--clr-text-dim); }
        ...
        .usr-stat:last-child { border-right: none; }
        .usr-stat:hover { background: rgba(255,255,255,0.02); }
        ...
        .usr-stat__num.red    { color: var(--clr-red); }
        .usr-stat__num.green  { color: var(--clr-success); }
        .usr-stat__num.blue   { color: var(--clr-info); }
        .usr-stat__num.dim    { color: var(--clr-text-dim); }
...
<div class="app-shell">
    ...
    <div class="app-main">
        ...
        <c:choose>
            <c:when test="${param.form eq 'true'}">
                <c:set var="pageTitle"    value="${modoEdicion ? 'Editar Usuario' : 'Nuevo Usuario'}" scope="request"/>
                <c:set var="pageSubtitle" value="Módulo de Usuarios" scope="request"/>
            </c:when>
            <c:otherwise>
                <c:set var="pageTitle"    value="Usuarios"                         scope="request"/>
                <c:set var="pageSubtitle" value="Gestión de accesos al sistema"    scope="request"/>
            </c:otherwise>
        </c:choose>
        ...
        <div class="page-content">
            ...
            <c:choose>
                    ...
                    <div class="module-header">
                        <div class="module-header__left">
                            ...
                            <div class="module-header__meta">
                                <span>Configura el acceso al sistema</span>
                        ...
                        <div class="form-card form-card--wide">
                            ...
                            <div class="form-card__header">
                                ...
                                <div>
                                    <p class="form-card__header-title">
                                        ${modoEdicion ? 'Datos del Usuario' : 'Nueva Cuenta de Acceso'}
                                    </p>
                                    <p class="form-card__header-sub">
                                        ...
                                        <c:if test="${not modoEdicion}">
                                            El ID se genera automáticamente · La contraseña se almacena cifrada con BCrypt
                                        </c:if>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="form-card__body">
                                ...
                                <div class="security-tip">
                                    ...
                                    <p>
                                        Las contraseñas se almacenan con cifrado
                                        <strong>BCrypt (costo 12)</strong>.
                                        Nunca se guardan en texto plano.
                                        Para editar un usuario existente, la contraseña <strong>no se modifica</strong>
                                        — usa "Resetear contraseña" desde la tabla si necesitas cambiarla.
                                    </p>
                                </div>

                                <%-- Sección: Credenciales --%>
                                <div class="form-section-divider">
                                    <span class="form-section-divider__label">Credenciales de acceso</span>
                                </div>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-field">
                                        ...
                                        <input type="password"
                                               ...
                                               maxlength="255"
                                               ${not modoEdicion ? 'required' : 'disabled'}
                                               autocomplete="new-password">
                                        <span class="form-field__hint">
                                            <c:choose>
                                                <c:when test="${modoEdicion}">
                                                    La contraseña actual se conserva. Para cambiarla usa "Resetear contraseña".
                                                </c:when>
                                                <c:otherwise>
                                                    Mínimo 8 caracteres. Se cifrará con BCrypt automáticamente.
                                                </c:otherwise>
                                            </c:choose>
                                        </span>
                                    </div>
                                ...
                                <%-- Sección: Rol y empleado --%>
                                <div class="form-section-divider">
                                    <span class="form-section-divider__label">Rol y empleado vinculado</span>
                                </div>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-field">
                                        <label for="idRol">
                                            Rol del sistema <span class="required-star">*</span>
                                        </label>
                                        <select id="idRol"
                                                name="idRol"
                                                class="form-control"
                                            ...
                                            <c:forEach var="rol" items="${roles}">
                                                <option value="<c:out value='${rol.id}'/>"
                                                    ${usuario.rol != null and usuario.rol.id eq rol.id ? 'selected' : ''}>
                                                    <c:out value="${rol.nombreRol}"/>
                                                    <c:if test="${not empty rol.descripcion}">
                                        ...
                                        <span class="form-field__hint">
                                            El rol determina qué módulos puede ver y usar el usuario
                                        </span>
                                    </div>

                                    <%-- Empleado vinculado --%>
                                    <div class="form-field">
                                        <label for="idEmpleado">Empleado vinculado</label>
                                        <select id="idEmpleado"
                                            ...
                                            <c:forEach var="emp" items="${empleados}">
                                                <option value="<c:out value='${emp.id}'/>"
                                                    ${usuario.empleado != null and usuario.empleado.id eq emp.id ? 'selected' : ''}>
                                                    <c:out value="${emp.apellido}"/>, <c:out value="${emp.nombre}"/>
                                        ...
                                        <span class="form-field__hint">
                                            Vincular permite al sistema identificar al empleado que usa la cuenta.
                                            Opcional pero recomendado.
                                        </span>
                                    </div>
                                </div>

                                <%-- Nota sobre accesos por rol --%>
                                <div class="module-alert module-alert--info" style="margin-bottom:0;">
                                    ...
                                    <div class="module-alert__body">
                                        <p class="module-alert__text">
                                            <strong>ROL-ADMIN</strong>: acceso completo · 
                                            <strong>ROL-RECEP</strong>: clientes, contratos, asistencia, membresías · 
                                            <strong>ROL-TRAINER</strong>: solo su panel de clases e inscripciones
                                        </p>
                                    </div>
                                </div>
                ...
                <%-- ──────────────────────────────────────────
                     VISTA: LISTA DE USUARIOS (default)
                     ────────────────────────────────────────── --%>
                <c:otherwise>
                        ...
                        <div class="module-header__left">
                            <h1 class="module-header__title">
                                ...
                                <span class="stat-chip">
                                    <c:out value="${totalUsuarios}"/>
                                </span>
                            </h1>
                            <div class="module-header__meta">
                                <span>Cuentas de acceso al sistema</span>
                            </div>
                        </div>
                        <div class="module-header__actions">
                    ...
                    <c:if test="${not empty usuarios}">
                        ...
                        <c:set var="cntInactivos" value="0"/>
                        <c:set var="cntAdmin"     value="0"/>
                        <c:set var="cntNoAdmin"   value="0"/>
                        <c:forEach var="u" items="${usuarios}">
                            <c:if test="${u.activo}">
                                <c:set var="cntActivos" value="${cntActivos + 1}"/>
                            </c:if>
                            <c:if test="${not u.activo}">
                                <c:set var="cntInactivos" value="${cntInactivos + 1}"/>
                            </c:if>
                            <c:if test="${u.rol != null and fn:contains(u.rol.id,'ADMIN')}">
                                <c:set var="cntAdmin" value="${cntAdmin + 1}"/>
                            </c:if>
                            <c:if test="${u.rol != null and not fn:contains(u.rol.id,'ADMIN')}">
                                <c:set var="cntNoAdmin" value="${cntNoAdmin + 1}"/>
                            </c:if>
                        </c:forEach>

                        <div class="usr-stats-strip">
                            <div class="usr-stat">
                                <span class="usr-stat__num"><c:out value="${totalUsuarios}"/></span>
                                <span class="usr-stat__label">Total cuentas</span>
                            </div>
                            <div class="usr-stat">
                                <span class="usr-stat__num green"><c:out value="${cntActivos}"/></span>
                                <span class="usr-stat__label">Activas</span>
                            </div>
                            <div class="usr-stat">
                                <span class="usr-stat__num dim"><c:out value="${cntInactivos}"/></span>
                                <span class="usr-stat__label">Inactivas</span>
                            </div>
                            <div class="usr-stat">
                                <span class="usr-stat__num red"><c:out value="${cntAdmin}"/></span>
                                <span class="usr-stat__label">Administradores</span>
                            </div>
                        </div>
                    </c:if>
                    ...
                    <div class="module-table-wrapper">
                        <c:choose>
                            <c:when test="${not empty usuarios}">
                                <table class="module-table" aria-label="Lista de usuarios del sistema">
                                    <thead>
                                        <tr>
                                            <th scope="col">Usuario</th>
                                            <th scope="col">Empleado vinculado</th>
                                            <th scope="col">Rol</th>
                                    ...
                                    <tbody>
                                        <c:forEach var="usr" items="${usuarios}" varStatus="loop">
                                            <c:set var="esMiCuenta" value="${usr.id eq sessionScope.userId}"/>
                                            <tr>
                                                    ...
                                                    <div class="email-cell">
                                                        ...
                                                        <div class="cell-name__info">
                                                            <span class="cell-name__primary">
                                                                ...
                                                                <c:if test="${esMiCuenta}">
                                                                    <span class="you-tag">Tú</span>
                                                                </c:if>
                                                            </span>
                                                            <span class="cell-id" style="margin-top:0.15rem;">
                                                                <c:out value="${usr.id}"/>
                                                            </span>
                                                        </div>
                                                    </div>
                                                ...
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${usr.empleado != null}">
                                                            <div class="usr-emp-info">
                                                                <span class="usr-emp-info__nombre">
                                                                    <c:out value="${usr.empleado.nombreCompleto}"/>
                                                                </span>
                                                                <span class="usr-emp-info__email">
                                                                    <c:if test="${usr.empleado.cargo != null}">
                                                                        <c:out value="${usr.empleado.cargo.nombre}"/>
                                                                    </c:if>
                                                                </span>
                                                            </div>
                                                        </c:when>
                                                ...
                                                <td>
                                                    <c:choose>
                                                        <c:when test="${usr.rol != null}">
                                                            <c:set var="rolId" value="${usr.rol.id}"/>
                                                            <c:choose>
                                                                <c:when test="${fn:contains(rolId,'ADMIN')}">
                                                                    <span class="rol-badge rol-badge--admin">
                                                                        <c:out value="${usr.rol.nombreRol}"/>
                                                                    </span>
                                                                </c:when>
                                                                <c:when test="${fn:contains(rolId,'RECEP')}">
                                                                    <span class="rol-badge rol-badge--recep">
                                                                        <c:out value="${usr.rol.nombreRol}"/>
                                                                    </span>
                                                                </c:when>
                                                                <c:when test="${fn:contains(rolId,'TRAINER')}">
                                                                    <span class="rol-badge rol-badge--trainer">
                                                                        <c:out value="${usr.rol.nombreRol}"/>
                                                                    </span>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <span class="rol-badge rol-badge--default">
                                                                        <c:out value="${usr.rol.nombreRol}"/>
                                                                    </span>
                                                                </c:otherwise>
                                                            </c:choose>
                                                ...
                                                <%-- Estado --%>
                                                <td>
                                                    <span class="estado-cuenta ${usr.activo ? 'activo' : 'inactivo'}">
                                                        <span class="estado-cuenta__dot"></span>
                                                        <c:out value="${usr.activo ? 'Activo' : 'Inactivo'}"/>
                                                    </span>
                                                </td>

                                                <%-- Resetear contraseña (inline form) --%>
                                                <td>
                                                    <form action="${pageContext.request.contextPath}/users"
                                                          method="post"
                                                          class="reset-pw-form"
                                                ...
                                                <td>
                                                    <div class="cell-actions">
                                                        ...
                                                        <%-- Si es mi cuenta: tooltip bloqueado --%>
                                                        <c:if test="${esMiCuenta}">
                                ...
                                <div style="display:flex; align-items:center; justify-content:space-between;
                                    ...
                                    <span style="font-size:0.75rem; color:var(--clr-text-dim);">
                                        ...
                                        usuario<c:if test="${totalUsuarios ne 1}">s</c:if> registrados
                                    </span>
                                    <a href="${pageContext.request.contextPath}/users?action=new"
                            ...
                            <c:otherwise>
                                <div class="table-empty">
                                    ...
                                    <p class="table-empty__desc">
                                        ...
                                        Primero registra el empleado desde el módulo de Empleados.
                                    </p>
                                    <a href="${pageContext.request.contextPath}/users?action=new"
                    ...
                    <c:if test="${not empty usuarios}">
                        <div style="margin-top:1rem; padding:0.85rem 1rem;
                            ...
                            <p style="font-size:0.73rem; color:var(--clr-text-dim); line-height:1.55;">
                                Las contraseñas se almacenan con cifrado <strong style="color:var(--clr-text-muted);">BCrypt (costo 12)</strong>.
                                No puedes desactivar tu propia cuenta. Para cambiar una contraseña, escríbela en el campo
                                "Resetear contraseña" y presiona el botón de candado. Mínimo 8 caracteres.
                            </p>
                        </div>
                    </c:if>
