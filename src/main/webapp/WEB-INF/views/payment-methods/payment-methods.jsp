<%-- ============================================================
     ...
     Módulo de gestión de métodos de pago del sistema.

     Servlet:  PaymentMethodsController.java  → GET/POST /payment-methods
     Acceso:   Solo ROL-ADMIN (RoleFilter)

     ── Atributos de request ─────────────────────────────────

     Vista lista (única vista):
       metodos        (List<MetodoPago>)  → todos: activos e inactivos
...
<head>
    ...
    <style>
        ...
        .pm-card.is-inactive::after{ background: var(--clr-text-dim); }
        ...
        .pm-card__status-label.activo  { color: var(--clr-success); }
        .pm-card__status-label.inactivo{ color: var(--clr-text-dim); }
        ...
        .pm-kpi:hover      { background: rgba(255,255,255,0.025); }
        ...
        <div class="page-content">
            ...
            <div class="module-header">
                <div class="module-header__left">
                    ...
                    <div class="module-header__meta">
                        <span>Gestiona las formas de cobro disponibles al crear contratos</span>
                ...
                <%-- No hay "Nuevo" porque el catálogo es fijo en BD --%>
            ...
            <c:if test="${totalActivos eq 1}">
                <div class="pm-only-active-banner" role="status">
                    ...
                    <p>
                        ...
                        No puedes desactivarlo porque se necesita al menos uno para
            ...
            <c:choose>
                <c:when test="${not empty metodos}">
                    ...
                    <div class="pm-grid">
                        <c:forEach var="mp" items="${metodos}" varStatus="loop">

                            <%-- Determinar clase CSS por ID del método --%>
                            <c:set var="pmClass" value="pm--default"/>
                            <c:if test="${fn:containsIgnoreCase(mp.id, 'EFECTIVO')}">
                                <c:set var="pmClass" value="pm--efectivo"/>
                            </c:if>
                            <c:if test="${fn:containsIgnoreCase(mp.id, 'VISA')}">
                                <c:set var="pmClass" value="pm--visa"/>
                            </c:if>
                            <c:if test="${fn:containsIgnoreCase(mp.id, 'MASTERCARD')}">
                                <c:set var="pmClass" value="pm--mastercard"/>
                            </c:if>
                            <c:if test="${fn:containsIgnoreCase(mp.id, 'YAPE')}">
                                <c:set var="pmClass" value="pm--yape"/>
                            </c:if>
                            <c:if test="${fn:containsIgnoreCase(mp.id, 'PLIN')}">
                                <c:set var="pmClass" value="pm--plin"/>
                            </c:if>
                            <c:if test="${fn:containsIgnoreCase(mp.id, 'TRANSFER')}">
                                <c:set var="pmClass" value="pm--transfer"/>
                            </c:if>
                            ...
                            <div class="pm-card ${pmClass}
                                ...
                                <div class="pm-card__icon-wrap">
                                    <div class="pm-card__icon">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${fn:containsIgnoreCase(mp.id,'VISA')
                                            ...
                                            <%-- Yape / Plin (QR / móvil) --%>
                                            <c:when test="${fn:containsIgnoreCase(mp.id,'YAPE')
                                ...
                                <div class="pm-card__body">
                                    ...
                                    <p class="pm-card__status-label ${mp.activo ? 'activo' : 'inactivo'}">
                                        <c:choose>
                                            <c:when test="${mp.activo}">Disponible para contratos</c:when>
                                ...
                                <div class="pm-card__footer">
                                    ...
                                    <%-- Caso especial: es el último activo → botón deshabilitado --%>
                                    <c:choose>
                                        <c:when test="${mp.activo and totalActivos eq 1}">
                                            <button type="button"
                                                ...
                                                Protegido (último activo)
                                            </button>
                                        </c:when>

                                        <%-- Activo → botón para desactivar --%>
                                        <c:when test="${mp.activo}">
                                            <form action="${pageContext.request.contextPath}/payment-methods"
                                                  method="post"
                                                  onsubmit="return confirm('¿Desactivar «${mp.nombre}»? Los contratos existentes no se verán afectados.');">
                ...
                <%-- Estado vacío (en teoría no debería ocurrir con un catálogo fijo) --%>
                <c:otherwise>
                    <div style="background:var(--clr-card); border:1px solid var(--clr-card-border);
                        ...
                        <div class="table-empty">
                            ...
                            <p class="table-empty__desc">
                                No hay métodos registrados en la base de datos.<br>
                                Verifica que el catálogo MetodosPago esté poblado en SQL Server.
                            </p>
                        </div>
                    </div>
                </c:otherwise>
            ...
            <div class="pm-info-footer">
                ...
                <p>
                    Solo los métodos <strong>activos</strong> aparecen disponibles al registrar nuevos contratos.
                    Desactivar un método no afecta los contratos ya existentes.
                    Debe mantenerse <strong>al menos un método activo</strong> en todo momento.
                    Los IDs son fijos (catálogo de BD) y no pueden crearse ni eliminarse desde aquí.
                </p>
            </div>
