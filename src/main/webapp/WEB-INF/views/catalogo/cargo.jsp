...
<div class="app-shell">
    ...
    <div class="app-main">
        <c:set var="pageTitle"    value="Cargos"                              scope="request"/>
        <c:set var="pageSubtitle" value="Gestión de cargos de empleados"      scope="request"/>
        ...
        <div class="page-content">
            ...
            <div class="module-header">
                <div class="module-header__left">
                    <h1 class="module-header__title">Cargos</h1>
                    <div class="module-header__meta">
                        <span>Gestiona los cargos del equipo del gimnasio</span>
                    </div>
                </div>
            </div>
            ...
            <%-- KPI strip + Tabla --%>
            <c:set var="cgActivos"   value="0"/>
            <c:set var="cgInactivos" value="0"/>
            <c:forEach var="cg" items="${cargos}">
                <c:if test="${cg.activo}">  <c:set var="cgActivos"   value="${cgActivos + 1}"/></c:if>
                <c:if test="${!cg.activo}"> <c:set var="cgInactivos" value="${cgInactivos + 1}"/></c:if>
            </c:forEach>
            <div class="cat-kpi-strip">
                <div class="cat-kpi">
                    <span class="cat-kpi__num">${fn:length(cargos)}</span>
                    <span class="cat-kpi__label">Total cargos</span>
                </div>
                <div class="cat-kpi">
                    <span class="cat-kpi__num green">${cgActivos}</span>
                    <span class="cat-kpi__label">Activos</span>
                </div>
                <div class="cat-kpi">
                    <span class="cat-kpi__num dim">${cgInactivos}</span>
            ...
            <div class="cat-table-wrapper">
                    <div style="display:flex; align-items:center; justify-content:space-between;
                        ...
                        <button type="button" onclick="openCargoModal()"
                    ...
                    <c:choose>
                        <c:when test="${not empty cargos}">
                            <table class="cat-table" aria-label="Cargos del sistema">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                ...
                                <tbody>
                                    <c:forEach var="cg" items="${cargos}" varStatus="loop">
                                        <c:set var="cgBadgeClass" value="av--default"/>
                                        <c:if test="${fn:containsIgnoreCase(cg.id,'ADM')}">    <c:set var="cgBadgeClass" value="av--red"/></c:if>
                                        <c:if test="${fn:containsIgnoreCase(cg.id,'REC')}">    <c:set var="cgBadgeClass" value="av--blue"/></c:if>
                                        <c:if test="${fn:containsIgnoreCase(cg.id,'TRAINER')}"><c:set var="cgBadgeClass" value="av--green"/></c:if>
                                        <tr>
                                            <td><span class="cell-id"><c:out value="${cg.id}"/></span></td>
                                            <td>
                                                <div class="cell-name">
                                                    <div class="cell-name__avatar ${cgBadgeClass}">
                                                        <c:out value="${fn:substring(cg.nombre,0,1)}"/>
                                                    </div>
                                                    <span style="font-weight:500; color:var(--clr-text); font-size:0.875rem;">
                                                        <c:out value="${cg.nombre}"/>
                                                    </span>
                                                </div>
                                            </td>
                                            <td>
                                                <span class="estado-pill ${cg.activo ? 'activo' : 'inactivo'}">
                                                    ${cg.activo ? 'Activo' : 'Inactivo'}
                                                </span>
                                            </td>
                                            <td>
                                                <div class="cell-actions">
                                                    <button type="button"
                                                       onclick="openCargoModal({id: '<c:out value='${cg.id}'/>', nombre: '<c:out value='${fn:escapeXml(cg.nombre)}'/>', estado: '${cg.activo ? 'activo' : 'inactivo'}'})"
                                                    ...
                                                    <form action="${pageContext.request.contextPath}/cargo"
                                                          method="post" class="toggle-estado-form">
                                                        <input type="hidden" name="action" value="toggle">
                                                        <input type="hidden" name="id"     value="<c:out value='${cg.id}'/>">
                                                        <input type="hidden" name="_csrf"  value="${sessionScope._csrfToken}">
                                                        <button type="button"
                                                                onclick="openActionModal(event, '${cg.activo ? 'deactivate' : 'activate'}', '${fn:escapeXml(cg.nombre)}')"
                                                                class="btn ${cg.activo ? 'btn-danger' : 'btn-success'} btn-sm btn-icon"
                                                                title="${cg.activo ? 'Desactivar' : 'Activar'}">
                                                            <c:choose>
                                                                <c:when test="${cg.activo}">
                                                    ...
                                                    <form action="${pageContext.request.contextPath}/cargo"
                                                        ...
                                                        <input type="hidden" name="_csrf"  value="${sessionScope._csrfToken}">
                                                        <button type="button"
                                                                onclick="openActionModal(event, 'delete', '${fn:escapeXml(cg.nombre)}')"
                                                                class="btn btn-danger btn-sm btn-icon"
                                                                title="Eliminar ${fn:escapeXml(cg.nombre)}">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                 viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div style="padding:0.72rem 1.35rem; border-top:1px solid var(--clr-border-light);
                                ...
                                <strong style="color:var(--clr-text-muted);">${fn:length(cargos)}</strong>
                                cargo(s) registrados
                            </div>
                        </c:when>
                        <c:otherwise>
                            <div class="table-empty">
                                ...
                                <p class="table-empty__title">Sin cargos registrados</p>
                                <p class="table-empty__desc">Crea el primer cargo para el equipo.</p>
                                <a href="${pageContext.request.contextPath}/cargo?action=new"
        ...
        <%-- ── Modal de Formulario ────────────────────────────────── --%>
        <div id="cargoModal" class="pm-modal-overlay">
            <div class="pm-modal">
                <div class="pm-modal__header">
                    <span id="cargoModalTitle">Nuevo Cargo</span>
                    <button class="pm-modal__close" type="button" onclick="closeCargoModal()">
                ...
                <form action="${pageContext.request.contextPath}/cargo" method="post" autocomplete="off">
                    <input type="hidden" name="action" value="save">
                    <input type="hidden" name="id" id="cargoModalId" value="">
                    <input type="hidden" name="_csrf" value="${sessionScope._csrfToken}">

                    <div style="padding: 1.5rem;">
                        
                        <div id="cargoModalIdContainer" style="display:none; margin-bottom: 1.25rem;">
                            <label style="display:block; font-size: 0.75rem; font-weight:600; color:var(--clr-text-dim); margin-bottom: 0.3rem;">
                                ID del Cargo
                            </label>
                            <div id="cargoModalIdDisplay" style="font-family:var(--font-mono); font-size:0.8rem; color:var(--clr-text-muted); background:var(--clr-surface); padding:0.6rem; border-radius:var(--radius-sm); border:1px solid var(--clr-border);">
                            </div>
                        </div>

                        <div style="margin-bottom: 1.25rem;">
                            <label for="cargoModalNombre" style="display:block; font-size: 0.75rem; font-weight:600; color:var(--clr-text-dim); margin-bottom: 0.3rem;">
                                Nombre del Cargo <span class="required-star">*</span>
                            </label>
                            <input type="text" id="cargoModalNombre" name="nombre" class="form-control"
                                   placeholder="Ej: Nutricionista, Coordinador"
                        ...
                        <div style="margin-bottom: 1.75rem;">
                            <label for="cargoModalEstado" style="display:block; font-size: 0.75rem; font-weight:600; color:var(--clr-text-dim); margin-bottom: 0.3rem;">
                                Estado
                            </label>
                            <select id="cargoModalEstado" name="estado" class="form-control" style="width: 100%; padding: 0.65rem 0.75rem; border-radius: var(--radius-md); border: 1px solid var(--clr-border); background: var(--clr-surface); color: var(--clr-text);">
                                <option value="activo">Activo</option>
                                <option value="inactivo">Inactivo</option>
                            </select>
                        </div>

                        <div class="module-alert module-alert--warning" style="margin-bottom:0; font-size:0.8rem; padding: 0.75rem;">
                            <svg class="module-alert__icon" xmlns="http://www.w3.org/2000/svg"
                                 fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="width:1.2rem; height:1.2rem;">
                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="module-alert__body">
                                <p class="module-alert__text" style="margin:0;">
                                    Los cargos Administrador, Recepcionista e Instructor tienen reglas especiales.
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            function openCargoModal(cargo) {
                ...
                const idInput = document.getElementById('cargoModalId');
                const idContainer = document.getElementById('cargoModalIdContainer');
                const idDisplay = document.getElementById('cargoModalIdDisplay');
                const nombreInput = document.getElementById('cargoModalNombre');
                const estadoSelect = document.getElementById('cargoModalEstado');

                if (cargo) {
                    title.textContent = 'Editar Cargo';
                    idInput.value = cargo.id;
                    idContainer.style.display = 'block';
                    idDisplay.textContent = cargo.id;
                    nombreInput.value = cargo.nombre;
                    estadoSelect.value = cargo.estado;
                } else {
            ...
            function closeCargoModal() {
                document.getElementById('cargoModal').classList.remove('is-open');
            }
            ...
            function openActionModal(event, type, methodName) {
                ...
                if (type === 'delete') {
                    ...
                    icon.textContent = '!';
                    text.innerHTML = `Se eliminará de forma permanente <br><strong><span style="color: var(--clr-text-muted);">` + methodName + `</span></strong><br><br><span style="font-size:0.8rem">Solo se permite si no tiene empleados asignados.</span>`;
                    ...
                    icon.textContent = '!';
                    text.innerHTML = `Se desactivará el cargo <br><strong><span style="color: var(--clr-text-muted);">` + methodName + `</span></strong>`;
                    btn.className = 'btn btn-primary';
                    ...
                    icon.textContent = '?';
                    text.innerHTML = `Se activará el cargo <br><strong><span style="color: var(--clr-text-muted);">` + methodName + `</span></strong>`;
                    btn.className = 'btn btn-primary';
